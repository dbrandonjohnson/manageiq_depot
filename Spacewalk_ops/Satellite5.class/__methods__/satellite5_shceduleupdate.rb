###################################
#
# CFME Automate Method: Satellite5_ScheduleUpdate
#
# by Kevin Morey
#
...
begin
  ...
  # Schedule vm updates from Satellite
  def scheduleupdate(vm, satellite, satellite_url, username, password)
    ...
    xmlrpc_client.call('system.scheduleupdate', xmlrpc_key, vm_systemid)
  end
  ...
  scheduleupdate(vm, satellite, satellite_url, username, password)

  #Apply Update Tag
  vm.tag_assign("SpaceWalkUpdatesAvailable/#{shortname}")

  #Apply custom attribute (last_update_check)
  m.custom_set(:Satellite_Last_Update_Scheduled, timestamp)
