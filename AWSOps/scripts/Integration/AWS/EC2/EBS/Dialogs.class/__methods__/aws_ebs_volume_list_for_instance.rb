...
begin
  ...
  log(:info, "HELPER#{ec2.volumes.inspect}")
  ...
  for ebs in ec2.volumes
    log(:info, "#{ebs.tags.name} - #{ebs.id} #{ebs.status == :available and ebs.availability_zone_name == instancezone}")
    #log(:info, "#{aws.provider_region} -> #{ebs.id}")
    ebs_hash[ebs.id] = ebs.id if ebs.status == :available and ebs.availability_zone_name == instancezone
  end
  log(:info, "#{ebs_hash}")
